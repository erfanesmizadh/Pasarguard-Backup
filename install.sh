...
# Author: AVASH_NET
# TCP MySQL, Telegram, Large Files Safe, Full Menu, Cron Setup
# =====================================================
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo
    read -p "Enter database name (or --all-databases): " DB_DISPLAY
    read -p "Enter Telegram Bot Token: " BOT_TOKEN
    read -p "Enter Telegram Chat ID: " CHAT_ID

    BACKUP_PATHS=("/opt/pasarguard" "/var/www/html")
...
remove_backups() {
    ...
    [[ "$confirm" == "yes" ]] && rm -rf "$BACKUP_DIR"/* && echo -e "${GREEN}[INFO] All backups removed.${RESET}" || echo -e "${YELLOW}[INFO] Operation cancelled.${RESET}"
}

# ---------------- Setup Cron ----------------
setup_cron() {
    echo -e "${CYAN}Configure Automated Backups${RESET}"
    echo "Select backup frequency:"
    echo "1) Every hour"
    echo "2) Every 2 hours"
    echo "3) Daily at specific time"
    echo "4) Weekly on specific day & time"
    read -p "Choice [1-4]: " freq
    ...
    case $freq in
        ...
        3)
            ...
            ;;
        4)
            read -p "Enter day of week (0-6, 0=Sun): " day
            read -p "Enter hour (0-23): " hour
            read -p "Enter minute (0-59): " minute
    ...
    echo "$CRON_TIME root bash $0 --auto" > "$CRON_FILE"
    chmod 644 "$CRON_FILE"
    systemctl restart cron
    echo -e "${GREEN}[INFO] Cron job set successfully! ✅${RESET}"
}
...
while true; do
    ...
    echo -e "${YELLOW}5.${RESET} Setup Automated Cron Backup"
